...
import List.Extra exposing (updateAt, removeAt)
import EveryDict exposing (EveryDict)
import Dict exposing (Dict)
import Helpers.DecimalField

import Models.Product exposing (Product, PriceGrid, AgePricing, Coverage(..), RiskLevel(..), DeductionMode, Tier, TierType(..), init)
    ...
    let
        setProduct p =
            let
                (coverage, risk) =
                    case p.focus of
                        Nothing -> (Employee, NormalRisk)
                        Just (_, coverage, risk) -> (coverage, risk)
                ...
                setDeductModePrices prices =
                    let
                        update =
                            \prices ->
                                case prices of
                                    Nothing -> Just (setRiskLevelPrice EveryDict.empty)
                                    Just p  -> Just (setRiskLevelPrice p)
                    in
                ...
                setBenefitPrices benefitPrices =
                    let update =
                        \prices ->
                            case prices of
                                Nothing -> Just (setDeductModePrices EveryDict.empty)
                                Just d -> Just (setDeductModePrices d)
                    in
                ...
                setAgePrices agePrices =
                    let update =
                        \prices ->
                            case prices of
                                Nothing -> Just (setBenefitPrices Dict.empty)
                                Just d -> Just (setBenefitPrices d)
                    in
                ...
                setCoveragePrices pricegrid =
                    let update =
                        \prices ->
                            case prices of
                                Nothing -> Just (setAgePrices Dict.empty)
                                Just d -> Just (setAgePrices d)
                    in
